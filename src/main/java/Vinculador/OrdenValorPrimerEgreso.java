...
public class OrdenValorPrimerEgreso implements CriterioVinculador {
    ...
    private String asignarEgrasosIngresos(ArrayList<IngresoEgreso> egresos, ArrayList<IngresoEgreso> ingresos) {
        ...
        int cantidadDeIngrsos=ingresos.size();
        String response="{\n" +
                "\"Relaciones\":[\n" +
                "{\n";
        for (int i = 0; i < cantidadDeIngrsos; i++) {
    ...
    private ArrayList<Integer> asignar(ArrayList<IngresoEgreso> egresos, IngresoEgreso ingreso) {
        ...
        egresos.forEach(egreso -> contador.set(this.sumarA(contador.get(), ingreso, egreso, response)));
    ...
    private int sumarA(int contador, IngresoEgreso ingreso, IngresoEgreso egreso, ArrayList<Integer> respuesta) {
    ...
    public void ordenarPorValor( ArrayList<IngresoEgreso> ingresosegresos ) {
        Collections.sort(ingresosegresos,new Comparator<IngresoEgreso>() {
            public int compare(IngresoEgreso ingEgre1, IngresoEgreso ingEgre2) {
                return ingEgre1.getMonto() - ingEgre2.getMonto();
            }
        });
    }
